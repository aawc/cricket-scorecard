...
- **Striker Marker**: Use clear, colorblind-accessible styling (such as an explicit `[STRIKER]` badge, `6px solid #0d6efd` accent border, and `#e7f1ff` background tint) to indicate the active striker. All interactive buttons must have a minimum 48px touch target for WCAG accessibility.
