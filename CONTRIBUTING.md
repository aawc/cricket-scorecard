...
| **v2 Event-Sourced Core & Analytics** | [`src/v2/`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/v2) | Delivery event streaming, pure statistical projection engine (`stats.ts`), SVG visual charts (`charts.ts`), hardware integrations (`hardware.ts`), CSV/JSON exporters (`export.ts`), and adapter bridge (`bridge.ts`). |
