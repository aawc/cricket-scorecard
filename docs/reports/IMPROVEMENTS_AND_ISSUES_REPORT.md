...
## 17. Milestone 19: Mid-Innings Batsman Substitution, Invariant Enforcement & Deserialization Healing

| Enhancement | Module | Description | Status |
| :--- | :--- | :--- | :--- |
| **Mid-Innings Batsman Substitution** | [`src/ui.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/ui.ts#L1358), [`src/reducer.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/reducer.ts#L310) | Batsman dropdowns and `CHANGE_BATSMAN` allow substituting batsmen at any time during active play (e.g. for injury, illness, Retired Not Out, or tactical changes). | `[PASS] Implemented` |
| **Active Striker Invariant Enforcement** | [`assignBatsmanToSlot`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/reducer.ts#L566) | Guarantees all batsmen not currently at the crease in Slot 1 or Slot 2 have `active: false`. Incoming batsmen inherit strike status based on the slot. | `[PASS] Implemented` |
| **Opener Stat Cleanup Before Ball 1** | [`src/reducer.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/reducer.ts#L348) | When changing opening batsmen before ball 1 of an innings, the previous player's unbatted 0/0 entry is cleaned up from `live.batsmen`. | `[PASS] Implemented` |
| **Mid-Innings Batsman Resumption** | [`src/reducer.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/reducer.ts#L310) | Players who retired not out can return to the crease later in the innings and resume their score from their previously recorded figures. | `[PASS] Implemented` |
| **Deserialization Active Flag Healing** | [`src/storage.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/storage.ts#L657) | `unminifyInnings()` heals active flags on restored permalinks so only players at the crease can have `active: true`, resolving ties to guarantee at most one active striker. | `[PASS] Implemented` |
| **Automated Regression Tests (Tests 96-99)** | [`test/test_cases.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/test/test_cases.ts#L3596) | Tests 96-99 verify batsman substitution during play, batsman resumption, opener cleanup at 0.0, and active flag healing on restored payloads. | `[PASS] Implemented` |

---

## 18. Milestone 20: Modal Scroll Lock Cleanup & Mobile Touch/Scroll Freeze Prevention (Bug 17)

When an innings concludes naturally (overs exhausted or all-out), `checkOverComplete()` emits a `SHOW_ALERT` UI event. In Bootstrap 5 environments, displaying the alert modal sets `document.body.style.overflow = 'hidden'`, `document.body.style.paddingRight = '...'`, and adds the `.modal-open` CSS class. Dismissing the modal synchronously hid elements and detached the `.modal-backdrop`, causing Bootstrap's internal transition listener to skip its `_scrollBar.reset()` teardown callback. Furthermore, native hide paths and chained modal workflows (such as `endInningsModal` transitioning to `alertDialogModal`) did not reset inline style properties on `document.body` or `document.documentElement`, leaving the body scroll-locked (`overflow: hidden`) permanently upon entering the second innings.

| Enhancement | Module | Description | Status |
| :--- | :--- | :--- | :--- |
| **Universal Modal Scroll Lock Purge** | [`cleanupModalScrollLock`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/modal.ts#L89) | Idempotently verifies whether any active modals remain visible (`.modal.show`, `display: block`, `display: flex`). If no modals are open, unconditionally strips `.modal-open` class, clears inline scroll locks (`overflow`, `overflowX`, `overflowY`, `paddingRight`, `marginRight`) from `document.body` and `document.documentElement`, and removes orphaned `.modal-backdrop` elements. | `[PASS] Implemented` |
| **Modal Dismissal & Event Integration** | [`closeModal`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/modal.ts#L131), [`initModalSystem`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/modal.ts#L203) | Integrated `cleanupModalScrollLock` into `closeModal()`, `nativeHideModal()`, the Escape key handler, dismissal button clicks, and Bootstrap's `hidden.bs.modal` listener. | `[PASS] Implemented` |
| **Defensive View Lifecycle Invariant** | [`src/ui.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/ui.ts#L167) | Defensively triggers `cleanupModalScrollLock()` on every UI render cycle ([`updateUI`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/ui.ts#L1300)), application initialization ([`initUI`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/ui.ts#L167)), second innings kickoff ([`startNextInnings`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/ui.ts#L1814)), and match reset ([`resetMatch`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/src/ui.ts#L1821)). | `[PASS] Implemented` |
| **Mock DOM Test Harness Enhancement** | [`test/test.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/test/test.ts#L97) | Extended Node test environment mock DOM with `body.style`, `documentElement.style`, `classList`, `appendChild`, and `querySelectorAll` for modals and backdrops. | `[PASS] Implemented` |
| **Automated Regression Suite (Test 100)** | [`test/test_cases.ts`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/test/test_cases.ts#L3858) | Test 100 verifies: (1) unit scroll unlock on `cleanupModalScrollLock()`, (2) full 1st-to-2nd innings transition with alert dismissal, (3) chained modal dismissal (`endInningsModal` -> `alertDialogModal`), and (4) complete state hydration and scroll unlock for the reported reproduction URL payload. | `[PASS] Implemented` |
| **Comprehensive Bug Diagnostic Report (Bug 17)** | [`docs/reports/BUG_REPORT.md`](file:///usr/local/google/home/vakh/git/hub/aawc/cricket-scorecard-pwa/docs/reports/BUG_REPORT.md#L415) | Documented full diagnostic context, environment specifications, root cause breakdown, resolution mechanics, and test evidence. | `[PASS] Implemented` |
